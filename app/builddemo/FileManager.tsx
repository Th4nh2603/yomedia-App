...
const FileManager: React.FC<FileManagerProps> = ({
  ...
  const handleRename = async (itemName: string) => {
    const newName = window.prompt("Nhập tên mới:", itemName);
    ...
    try {
      const res = await fetch("/api/sftp/rename", {
        ...
        body: JSON.stringify({
          path: currentPath, // prop / state hiện tại
          oldName: itemName,
      ...
      if (!data.success) {
        alert("Rename lỗi: " + data.error);
        return;
      }

      alert(`Đổi tên thành công: ${itemName} → ${newName.trim()}`);

      // 🔁 reload lại list
      loadFiles(currentPath);
    } catch (err: any) {
      alert("Có lỗi khi rename: " + err.message);
    }
  };
          ...
          <tbody>
            {files.map((item, index) => (
              ...
              >
                ...
                {/* 👇 Cột actions: nút Rename */}
                <td className="px-4 py-2 text-right">
                  <button
                    onClick={(e) => {
                      e.stopPropagation(); // không trigger onClick của cả hàng
